...
class PumpEnvVar_Sequence(PumpEnv):
    def __init__(self, load_range=[0.0,2.0], goal_pressure_range=[1.1, 2.0], goal_sequence_length = 10):
    ...
    def generate_moving_goal_pressure(self, range):
            x = np.arange(0, self.goal_sequence_length, 1)
            phase_noise = np.random.uniform(0,np.pi, (1,)) 
            amp_noise = np.random.uniform(-1,1, (10,))
            y = 0.25*np.sin(x+phase_noise) + 0.25*amp_noise
    ...
    def reset(self):
        ...
        self.goal_pressure_sequence = self.generate_moving_goal_pressure(self.goal_pressure_range) * P_0
        self.goal_sequence_number = 0
        self.goal_pressure = self.goal_pressure_sequence[self.goal_sequence_number]
